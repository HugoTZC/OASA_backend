...
router.get('/', async (req, res) => {
  try {
    ...
    // Add sorting - Featured products first, then by user's sort preference
    ...
    if (validSortFields.includes(sortBy) && validSortOrders.includes(sortOrder.toLowerCase())) {
      ...
      // Featured products first using boolean DESC (true comes before false)
      query += ` ORDER BY p.is_featured::int DESC, p.${sortField} ${sortOrder.toUpperCase()}`;
    } else {
      // Featured products first, then by name
      query += ` ORDER BY p.is_featured::int DESC, p.name ASC`;
    }
